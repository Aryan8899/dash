...
const TickerRow = memo(({ label, items, tickerType }: TickerRowProps) => {
  ...
  const labelColor =
    tickerType === "crypto" ? "text-blue-600" : "text-pink-600";
  const valueColor =
    tickerType === "crypto" ? "text-blue-500" : "text-pink-500";
  const value = tickerType === "crypto" ? "text-blue-500" : "text-blue-500";

  return (
    <div className="p-2 relative overflow-hidden border border-black rounded-2xl bg-white dark:bg-transparent">
      <div
        className={`flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 bg-${bgColor}/10 py-2 px-4 rounded-2xl`}
      >
        <span className={`${labelColor} font-medium min-w-[80px]`}>
          {label}
        </span>
        <div id={`container-${label}`} className="flex-1 overflow-hidden w-full">
          <div
            id={`ticker-${label}`}
...
const Dashboard = () => {
  ...
  return (
    <div className="w-full min-h-screen custom-gradient p-2 sm:p-4 font-poppins space-y-4 sm:space-y-8">
      {/* Navbar */}
      <nav className="relative flex items-center justify-between w-full mb-4 sm:mb-6">
        <div className="flex items-center gap-2 bg-white rounded-full border border-gray-200 shadow-lg px-2 py-1  md:px-4 md:py-2">
          <img
        ...
        <div className="hidden lg:flex items-center space-x-6">
          ...
          <div className="flex items-center space-x-3">
            <button className="flex items-center space-x-2 text-gray-600 hover:text-gray-900 bg-white rounded-2xl border border-gray-200 shadow-md py-2 px-3">
        ...
        <div className="lg:hidden flex items-center gap-2">
          <button className="flex items-center space-x-2 text-gray-600 hover:text-gray-900 bg-white rounded-full border border-gray-200 shadow-md py-1 px-2 text-xs sm:text-sm">
            <span>Connectwallet</span>
          </button>
          {/* Hamburger Menu Button */}
          <button
            id="hamburger-button"
            className="p-1 sm:p-2 border border-gray-300 rounded-md bg-white z-50"
            onClick={toggleMenu}
        ...
        <div
  id="mobile-nav"
  className={`lg:hidden fixed top-0 left-0 right-0 bottom-0 bg-white transform transition-transform duration-300 ease-in-out z-40 ${
    menuOpen ? "translate-x-0" : "translate-x-full"
  } overflow-y-auto overflow-x-hidden`}
>
  <div className="flex flex-col p-4 space-y-4 mt-16">
    <button className="px-2 py-2 bg-gray-900 text-white rounded-full text-sm font-medium w-full">
      Dashboard
    </button>
    {["People", "Hiring", "Devices", "Apps", "Salary"].map((item) => (
      <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-6">
        {/* Total Bonus Section */}
        <div
          className="col-span-1 bg-transparent backdrop-blur-md text-black p-6 flex flex-col items-center rounded-2xl border border-black animate-fade-left w-full mobile-center "
          style={{
       ...
       {/* Bonus Details Section */}
<div className="col-span-4 p-4 rounded-2xl shadow-md border border-black bg-transparent animate-fade-left">
  ...
  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
    {[
      ...
      <div
        key={bonus.title}
        className="flex flex-col items-center text-center space-y-2 p-4 rounded-lg border border-gray-200 shadow-md"
      >
        <CircularProgress
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6 animate-fade-left">
        {[
          {
            title: "E-Wallet",
            value: "2389.56",
          ...
          <div
            key={item.title}
            className="text-black p-6 rounded-2xl border border-black"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6 animate-fade-left">
        {[
          {
            title: "Game Wallet",
            value: "570",
          ...
          <div
            key={item.title}
            className="text-black p-6 rounded-2xl text-center border border-black"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-left">
        {/* Global Pool */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6 animate-fade-left">
        {/* Left Column */}
        <div className="space-y-6">
          {/* Referral Link Section */}
         {/* Referral Link Section */}
<div className="text-black p-4 rounded-2xl border border-black">
  <h3 className="text-black text-base mb-2">Referral Link</h3>
  <div className="flex flex-col sm:flex-row items-center gap-4">
    <img
      src="/image.png"
      alt="QR Code"
      className="w-24 h-24 border border-gray-200 flex-shrink-0"
    />
    <div className="flex flex-col gap-2 flex-grow w-full">
      <div className="flex flex-col sm:flex-row items-center justify-between w-full">
        <span className="text-sm break-words w-full sm:w-auto">
          https://cticlub.org/register/624515f380c05
        </span>
        <button className="px-3 py-1 bg-yellow-500 text-white rounded-md text-sm w-full sm:w-auto mt-2 sm:mt-0">
          Copy Link
        </button>
      </div>
      <p className="text-gray-400 text-sm text-center sm:text-left">
          ...
          <div className="text-black p-4 rounded-2xl border border-black">
            ...
            <div className="flex gap-4 items-center">
              <input
                ...
                className="flex-grow text-black bg-transparent p-3 rounded-2xl border border-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
        ...
        <div className="text-black p-2 rounded-2xl border border-black h-full overflow-y-auto">
          <h3 className="text-sm font-semibold mb-1">Recent Bonus</h3>
          <div className="space-y-1">
            {recentBonuses.map((bonus, index) => (
              <div
                key={index}
                className="flex items-center justify-between p-2 rounded-2xl border animate-fade-in-up"
              >
      ...
      <div className="text-black mt-6 border border-black animate-fade-left">
        <div className="text-black overflow-hidden">
          <div className="grid grid-rows-2 divide-y text-black">
            <TickerRow label="CRYPTO" items={cryptoData} tickerType="crypto" />
      ...
      <div className="mt-6 text-center text-black animate-pulse">
